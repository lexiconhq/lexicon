...
export function PollPreview(props: Props) {
  ...
  const { poll: pollData, pollVotes, isCreator, postId, topicId } = props;
  ...
  const getPercentage = (value: number, total: number) =>
    value === 0 ? 0 : Math.round((value / total) * 1000) / 10;
  ...
  const result = () => {
    ...
    if (chartType === PollChartType.Bar && !showResult) {
      return (
        <View style={styles.numberRatingResult}>{resultInfo('textLight')}</View>
      );
    }

    if (chartType !== PollChartType.Pie) {
      return null;
    }
...
const useStyles = makeStyles(({ spacing, colors, graphSizes }) => ({
  ...
  shownNumberRatingResult: { backgroundColor: colors.lightBackgroundDarker },
  hiddenNumberRatingResult: {
    backgroundColor: colors.lightTextDarkest,
    height: 125,
  },
